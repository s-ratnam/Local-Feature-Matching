...
def angles_to_vectors_2d_pytorch(angles: torch.Tensor) -> torch.Tensor:
    ...
    angle_vectors = torch.stack((torch.cos(angles), torch.sin(angles)), dim = 1)
...
class SubGridAccumulationLayer(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.layer = nn.Conv2d(in_channels = 8, out_channels = 8, kernel_size = 4, padding = 2,bias = False,groups = 8)
